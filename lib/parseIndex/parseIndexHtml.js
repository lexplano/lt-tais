var assert = require("assert"),
	...
	parseIndexMetaCell = require("./parseIndexMetaCell");
...
function parseRow(row) {
	...
	return {
		...
		docType: docType,
		docNo: docNo,
		meta: parseIndexMetaCell(infoCell.innerHTML),
		metaHtml: infoCell.innerHTML
	};
}
...
function parseFile(fn, html, cb) {
	...
	tick.start("dom");
	var promise = Q.ninvoke(jsdom, "env", html, [], {features: {FetchExternalResources: []}})
...
if (require.main === module) {
	process.stdin.pipe(concat(function (buf) {
		parseFile("stdin", iconv.decode(buf, CHARSET), function (err, res) {
			if (err) throw err;
			console.log(require("util").inspect(res, {depth: 3}));
		});
	}));
}
